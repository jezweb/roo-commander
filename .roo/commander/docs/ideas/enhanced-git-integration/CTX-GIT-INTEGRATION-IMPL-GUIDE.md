...
*   Create/Update `BROKEN_LINK_NEEDS_MANUAL_REVIEW_.roo/rules/08-git-commit-standard.md` with the full commit message standard (Conventional Commits + detailed body + mandatory task reference footer).
*   Create `BROKEN_LINK_NEEDS_MANUAL_REVIEW_.roo/rules/09-git-branching-standard.md` defining the task branch naming convention (`type/TASK-ID-description`).
*   Create `BROKEN_LINK_NEEDS_MANUAL_REVIEW_.roo/rules/10-git-tagging-standard.md` defining release tag format (`vX.Y.Z`, annotated) and purpose.
*   Create `BROKEN_LINK_NEEDS_MANUAL_REVIEW_.roo/rules/11-git-workflow-integration-policy.md` defining the policy/heuristics for *when* Commander/Leads should propose creating task branches or initiating release tagging.
...
*   **KB (`.roo/commander/modes/dev-git/kb/`):**
...
*   **Rules (`.roo/rules-roo-commander/`):**
    ...
    *   In `BROKEN_LINK_NEEDS_MANUAL_REVIEW_.roo/commander/modes/roo-commander/rules-roo-commander/03-delegation-procedure-rule.md`: Add step to consider creating a task branch (following Rule `11` policy) before complex MDTM delegation.
    *   In appropriate workflows (e.g., build/release): Add steps to initiate release tagging following Rule `10` & `11`.
*   **KB (`.roo/commander/modes/roo-commander/kb/`):**
...
*   Create/Update `BROKEN_LINK_NEEDS_MANUAL_REVIEW_.roo/commander/docs/standards/git-workflows.md` detailing the commit, branch, and tag standards.
